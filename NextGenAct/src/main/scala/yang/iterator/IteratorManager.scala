...
/**
  ...
  */
class IteratorManager[T](corbaObjInitiator: CorbaObjInitiator,
  ...
  override def receive: Receive = {
    case RequestCreateIteratorIor(iteratorId) =>
      ...
      val isOK = true
      sender ! reply_get_alarm_list(isOK, alarmIterator)
  ...
  def createNewIterator(iteratorIndex: Int): ActorRef = {
    ...
    val actorRef = context.actorOf(Props.create(classOf[IteratorActor[T]],
