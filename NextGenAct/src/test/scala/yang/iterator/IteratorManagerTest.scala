...
class IteratorManagerTest extends TestKitAndFunSuite {
  ...
  test("testCreateNewIterator") {
    ...
    val iteratorMgrActorRef: TestActorRef[IteratorManager[Any]] = TestActorRef(Props(new IteratorManager[Any](mockCorbaObj, mockDataPoll)))
  ...
  test("test for iterator timeout") {
    val mockCorbaObj = new MockCorbaObjManager
    val mockDataPoll = new DonothingPoller
    val iteratorMgrActorRef: TestActorRef[IteratorManager[Any]] = TestActorRef(Props(new IteratorManager[Any](mockCorbaObj,
  ...
  class MockCorbaObjManager extends CorbaObjInitiator {
    override def active(toActive: Servant) {}

    override def deActive(toDeactive: Servant) {}
  }
