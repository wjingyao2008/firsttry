...
class FieldSelectorFilterTest extends FunSuite with Matchers {
  ...
  test("testSelfPass") {
    ...
    val fieldSelectorFilter: Filter[Data] = new FieldSelectorFilter[Data](new DataValueIntSelector, OperatorEnum.<=, "4")
    var data = Data(3, "1!")
  ...
  test("test for string '4' should not == '1!") {
    val data = Data(4, "1!")
    val strFieldSelectorFilter: Filter[Data] = new FieldSelectorFilter[Data](new DataValueStringSelector, OperatorEnum.==, "4")
  ...
  test("test for string '4' should == '41") {
    val strFieldSelectorFilter: Filter[Data] = new FieldSelectorFilter(new DataValueStringSelector, OperatorEnum.==, "4")
    val data2 = Data(3, "4")
    strFieldSelectorFilter.isPass(data2) shouldBe true
  }
  ...
  test("testSelfPass for type string") {
    ...
    var strFieldSelectorFilter: Filter[Data] = new FieldSelectorFilter[Data](new DataValueStringSelector, OperatorEnum.==, "4")
    var data = Data(3, "1!")
    strFieldSelectorFilter.isPass(data) shouldBe false
...
class DataValueStringSelector extends ValueSelector[Data] {
  ...
  override def createBasicTypeFilter(equator: Equator, value: String): BasicFilter = new StringFilter(equator, value)
}

class DataValueIntSelector extends ValueSelector[Data] {
  override def getVal(name: Data): Any = name.a

  override def createBasicTypeFilter(equator: Equator, value: String): BasicFilter = new IntegerFilter(equator, value)
}